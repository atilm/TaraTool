...
class TaraDocumentGenerator:
    ...
    def _build_threat_scenario_table(self, tara: Tara) -> MarkdownTable:
        builder = MarkdownTableBuilder() \
            .withHeader("ID", "Threat Scenario", "Initial Risk", "Residual Risk")
        ...
        for asset in tara.assets:
            for security_property, damage_scenario_ids in asset.damage_scenarios.items():
                for ds_id in damage_scenario_ids:
                    ...
                    residual_feasibility: Feasibility = feasibilities[feasibility_id].residual
                    residual_feasibility_level = residual_feasibility.calculate_feasibility_level()
                    residual_risk = RiskLevel.look_up(impact, residual_feasibility_level)
